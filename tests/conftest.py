...
def app_config():
    ...
    return {"debug": True, "testing": True, "secret_key": "test-secret-key"}
...
def client(app_config):
    """
    Fixture pytest que fornece um Flask test client.

    Tenta usar a factory `create_app()` (app.app). Se não for possível, tenta importar
    ...
    try:
        ...
        try:
            # Fallback para instância app já criada
            from app.app import app as _app
        except Exception:
            raise

    # Aplicar overrides de configuração para testes
    _app.config.update(app_config)

    # Fornecer client dentro do contexto da aplicação
