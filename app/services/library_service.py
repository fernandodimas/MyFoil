...
class LibraryService:
    ...
    def get_game_details(title_id: str, app_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if not title_obj:
            result = info.copy()
            result.update({
                'id': tid, 'app_id': tid, 'owned_version': 0,
                'has_base': False, 'has_latest_version': False, 'has_all_dlcs': False, 'owned': False,
                'files': [], 'updates': [], 'dlcs': [], 'category': info.get('category', [])
            })
        ...
        for b in base_apps:
            ...
            for f in app_model.files:
                base_files.append({
                    'id': f.id, 'filename': f.filename, 'filepath': f.filepath,
                    'size': f.size, 'size_formatted': format_size_py(f.size), 'version': app_model.app_version
                })
        ...
        for dlc_id in dlc_ids:
            ...
            dlcs_list.append({
                ...
                'owned': owned, 'release_date': dlc_info.get('release_date', ''),
                'files': files
            })

        result = info.copy()
        result.update({
            'id': tid, 'app_id': tid, 'title_id': tid,
        ...
        total_size = sum(f.size for a in all_title_apps if a['owned']
                        for f in db.session.get(Apps, a['id']).files)
        result.update({
            'size': total_size,
            'size_formatted': format_size_py(total_size),
            'status_color': 'orange' if result['has_base'] and (not result['has_latest_version'] or not result['has_all_dlcs'])
    ...
    def get_unidentified_files() -> List[Dict[str, Any]]:
        ...
        results = [{
            'id': f.id, 'filename': f.filename, 'filepath': f.filepath,
            'size': f.size, 'size_formatted': format_size_py(f.size),
